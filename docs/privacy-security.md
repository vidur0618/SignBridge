...
The app does not mount the remote SDK merely because a token exists. Captions-only is the default; a staff member must select the experimental mode and review each exact finalized or explicitly submitted message. `POST /api/avatar/drafts` safety-gates and normalizes the text into a session-bound, five-minute in-memory draft without contacting Hand Talk. `POST /api/avatar/drafts/:draftId/decision` consumes that draft exactly once; only `play` releases its canonical text with a random authorization ID. Structured events contain no transcript. This prevents a decision request from replacing the text, but it does not prove a human clicked the control or force the browser SDK to send the authorized text; see the [server-owned draft protocol](./avatar-draft-protocol.md). An allowed message is sent directly from the browser, and the CSP permits provider connections to `handtalk.me`, `*.handtalk.me`, and `wss://*.handtalk.me`. The repository has not verified which endpoints, subprocessors, regions, telemetry, or retention rules the v1 SDK uses. Hand Talk's separate [plugin data-collection documentation](https://docs.handtalk.me/en/docs/duvidas-erros-limitacoes/coleta-de-dados/) says that plugin metrics can be disabled only partly and that translated-word totals remain processed; do not assume that page defines the v1 SDK's behavior. Obtain SDK-specific written terms and a DPA before entering customer text.

import { LiveServerEventSchema, type LiveSessionConfig } from "@signbridge/contracts";
    ...
    this.connectPromise = new Promise((resolve, reject) => {
      ...
      socket.onmessage = (message) => {
        ...
        try {
          ...
          this.options.onEvent({ type: "error", code: "invalid_server_event", message: "The speech connection returned an invalid event." });
        }
      };
      socket.onerror = () => {
        if (!opened) fail(new Error("Could not connect to live transcription."));
      };
      socket.onclose = () => {
        this.socket = null;
        if (this.intentionallyClosed) {
